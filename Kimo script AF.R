...
###### AF TESTING: Has this visual inspection happened?? If so remove top comment #########
###### IF THIS IS A MANUALLY MADE FILE I CAN JUST RENAME THE LENGTH PART MANUALLY TOO #####
# now need to visually inspect all kinases that have more than 1 of each motif, so that can call which one
# SOURCE_motif2use is a file with the correct motif to use when there are multiple. Correct motif 
motifToUse     <- read.csv ('SOURCE_motif2use.csv')
genbankkinases<- merge    (genbankkinases, motifToUse, by = c('f','Gene', 'Length_i_think', 'Protein_Seq', 'VAIKmotif', 'HRDmotif', 'DFGmotif'))

#####################################################
#clean up genbankkinases
#AF: Hoping to remove this section
#colnames(genbankkinases)[2] <- "Gene"
colnames(genbankkinases)[3] <- "Length"             #If you changed source column names from "Length-i-think" to Length it wouldn't need this. AND would look better
...
#write.csv(genbankkinases, file = "genbankkinases5.csv") # AF testing
######################################################

# Locate all motifs
##### AF Testing: Check to see if this is needed and if not remove it #########
HRDmotifs  <- as.vector(genbankkinases$HRDmotif)
VAIKmotifs <- as.vector(genbankkinases$VAIKmotif)
DFGmotifs  <- as.vector(genbankkinases$DFGmotif)

######################  MORE TESTING NEEDED ON THIS SECTION ####################
# Don't need locate all for VAIK as there is only one gene with 2 vaik motifs (map4k1) and the valid one is the first
##### Can you justify this statement as it doesn't quite seem correct, needs additional justification/ explaination!!
#locates sequence position of first VAIK motif
...
#same function as second HRD motif locator but for 3rd motif
beyondsecondHRD <- substr(genbankkinases$Protein_Seq, secondHRD + 1, endofstring) #prints the rest of the sequence after the motif
...
# Determines the correct DFG to use
...
# Determines the correct HRD to use
...
########################
# Finds the HRD-6 motif
########################

# Finds sequence location around the APE motif
...
# Finds amino acid identity for above sequence locations
...
# Looks for a H in HRD-6, then -7, -8, -4, -9, -5 and -10 
#If it cannot find one in these locations, it will output HRD-6
...
# Determines an APE fragment based upon the DFG motif. Fragment houses the APE and GxGxxG motifs
...
# Outputs the final APE motif sequence information
...
# Looks for a G in APE-6, then APE-9, APE-5, APE-8, APE-10 and APE-11. 
#If it cannot find one in these locations, it will output APE-6
...
# find GxGxxG
############ AF: Does this find only the first one? What happens if there are multiple domains?? ##############
...
# This is used to determine the Salt Bridge E
...
########################
# Calculates and outputs the number of mutations around these positions in from the TCGA database 
#### NAT: Can you help reword this better
########################
...
#merge CCLE and TCGA
######## NOT UNDERSTANDING WHAT THIS DOES WITH RESPECT TO THE REST OF THE SCRIPT #############
### do you mean to merge this with Freqscreen (the data we have made) or just simply merge the TCGA and CCLE databases??
