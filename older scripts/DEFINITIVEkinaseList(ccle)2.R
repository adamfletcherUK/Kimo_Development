...
#now need to keep only transcripts that are used by CCLE
ccletranscripts <- read.csv ( file = 'SOURCE_Correct_CCLE_Transcripts.csv')
genbank = merge(ccletranscripts, genbank, by = 'f', nomatch = 0)
...
unique(uniqkinase)
# this displays 420 unique kinase names in CCLE
...
#end of kinase domain = APE + 40
genbankkinases$endCAT <- as.numeric(genbankkinases$APE_E1loc + 40)
...
write.csv (genbankkinases, 'TEMP_check.csv')


# merge with combined CCLE data
comboDATA <- read.csv (file = 'SOURCE_ccletrimmed_namechanged.csv')
colnames(comboDATA)[8] <- "case_id"
...
merged_G1 <- merged
write.csv (merged_G1, file='outputs/ccle/merged_GxGxxG_G1.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_G2 <- merged
write.csv (merged_G2, file='outputs/ccle/merged_GxGxxG_G2.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_K <- merged
write.csv (merged_K, file='outputs/ccle/critical_lysine.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_D <- merged
write.csv (merged_DFG_D, file='outputs/ccle/merged_DFG_G.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_F <- merged
write.csv (merged_DFG_D, file='outputs/ccle/merged_DFG_F.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_G <- merged
write.csv (merged_DFG_G, file='outputs/ccle/merged_DFG_G.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_HRD_H <- merged
write.csv (merged_HRD_H, file='outputs/ccle/merged_HRD_H.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_HRD_R <- merged
write.csv (merged_HRD_R, file='outputs/ccle/merged_HRD_R.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_HRD_D <- merged
write.csv (merged_HRD_D, file='outputs/ccle/merged_HRD_D.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_APE_A <- merged
write.csv (merged_APE_A, file='outputs/ccle/merged_APE_A.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_APE_P <- merged
write.csv (merged_APE_P, file='outputs/ccle/merged_APE_P.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_APE_E <- merged
write.csv (merged_APE_E, file='outputs/ccle/merged_APE_E.csv')
...
write.csv (merged_correctedG, file='outputs/ccle/merged_correctedG.csv')


#bind all data tables together
total <- rbind (merged_G1, merged_G2, merged_K, merged_HRD_H, merged_HRD_R, merged_HRD_D, merged_DFG_D, merged_DFG_F, merged_DFG_G, merged_APE_A, merged_APE_P, merged_APE_E)
write.csv (total, file ='outputs/ccle/total.csv')

freqscreen <- table(total$Gene)
write.csv (freqscreen, file ='SOURCE_cclefreqscreen.csv')         

#merge CCLE and TCGA
tcgafreq <- read.csv ( file = 'SOURCE_tcgafreqscreen.csv')
cclefreq <- read.csv ( file = 'SOURCE_cclefreqscreen.csv')
totalfreq = merge(tcgafreq, cclefreq, by = 'Var1', nomatch = 0)
write.csv (totalfreq, file ='TEMP_totalfreqscreen.csv')


# TRUNCATING PART
mutDATA <- read.csv (file = 'SOURCE_ccletrimmed_namechanged.csv')
...
# extract predicted amino acid form each protein seq (calculated from Codon)
merged$codonConfirm <- substr(merged$Protein_Seq, merged$Codon, merged$Codon)
write.csv (merged, file ='TEMP_truncscreenPREfilter.csv')


#merge by Codon smaller than endCAT and Ref Amino acid is the same as locAmino
truncSTRINGENT <- subset (merged, (Codon <= endCAT) & (RefAmino == codonConfirm))
truncFLEXIBLE <- subset (merged, (Codon <= endCAT))


#to filter out duplicates of Gene, Codon and Sample
truncSTRINGENT$duplicateREF <- paste(truncSTRINGENT$Gene, truncSTRINGENT$Codon, truncSTRINGENT$endCAT, truncSTRINGENT$case_id, sep ='_')
truncSTRINGENT$mergedDUP= !duplicated(truncSTRINGENT$duplicateREF)
genefreq <- subset(truncSTRINGENT, mergedDUP == 'TRUE')
write.csv (genefreq, file ='TEMP_stringentscreen.csv')

truncFLEXIBLE$duplicateREF <- paste(truncFLEXIBLE$Gene, truncFLEXIBLE$Codon, truncFLEXIBLE$endCAT, truncFLEXIBLE$case_id, sep ='_')
truncFLEXIBLE$mergedDUP= !duplicated(truncFLEXIBLE$duplicateREF)
genefreqFLEXIBLE <- subset(truncFLEXIBLE, mergedDUP == 'TRUE')
write.csv (genefreqFLEXIBLE, file ='TEMP_flexscreen.csv')

freqSTRINGENT <-table(genefreq$Gene)
write.csv (freqSTRINGENT, file ='TEMP_freqSTRINGENT.csv')


freqFLEX <-table(genefreqFLEXIBLE$Gene)
write.csv (freqFLEX, file ='TEMP_freqFLEXIBLE.csv')


#to length correct
freq <- read.csv ('SOURCE_freqTRUNCccle.csv')
colnames(freq)[2] <- "Gene"
matches <- merge(freq, genefreq, by = 'Gene')
# then multiply freq by 1/endCAT to get length corrected freq
matches$oneoverendCAT <- 1 / matches$endCAT
matches$score <- matches$Freq * matches$oneoverendCAT
matches$DUP= !duplicated(matches$Gene)
matches <- subset(matches, DUP == 'TRUE')
write.csv (matches, file ='SOURCE_lengthcorrectedfreq_ccle.csv')
