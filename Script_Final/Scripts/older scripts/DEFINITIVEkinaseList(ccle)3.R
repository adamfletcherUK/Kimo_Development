...
#end of kinase domain = APE + defined end point
...
comboDATA <- read.csv (file = 'SOURCE_ccletrimmed_namechanged_HRT18removed.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_D <- merged
write.csv (merged_DFG_D, file='outputs/ccle/merged_DFG_G_all.csv')
...
merged <- subset(merged, DUP == 'TRUE')
merged_DFG_F <- merged
write.csv (merged_DFG_D, file='outputs/ccle/merged_DFG_F_all.csv')
...
# TRUNCATING PART
mutDATA <- read.csv (file = 'SOURCE_ccletrimmed_namechanged_HRT18removed.csv')
...
write.csv (merged, file ='TEMP_check_ccle-1_all.csv')
...
truncSTRINGENT$duplicateREF <- paste(truncSTRINGENT$Gene, truncSTRINGENT$Codon, truncSTRINGENT$case_id, sep ='_')
truncSTRINGENT$mergedDUP= !duplicated(truncSTRINGENT$duplicateREF)
genefreq <- subset(truncSTRINGENT, mergedDUP == 'TRUE')
write.csv (genefreq, file ='TEMP_stringentscreen_ccle_all.csv')


freqSTRINGENT <-table(genefreq$Gene)
write.csv (freqSTRINGENT, file ='TEMP_freqTRUNCccleALL.csv')



#to length correct
freq <- read.csv ('TEMP_freqTRUNCccleALL.csv')
...
write.csv (matches, file ='TEMP_lengthcorrectedfreq_ccleALL.csv')